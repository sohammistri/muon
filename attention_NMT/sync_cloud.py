...
def sync_checkpoints() -> None:
    """
    For each d6-* checkpoint folder, compute the diff against the remote and
    upload only new files using bbb cp.
    """
    pattern = os.path.join(CHECKPOINT_BASE, "d6-*")
    ckpt_dirs = sorted(glob.glob(pattern))

    if not ckpt_dirs:
        log.info("No d6-* checkpoint dirs found under %s — skipping.", CHECKPOINT_BASE)
        return
