...
from GPT2.common import compute_init, autodetect_device_type
...
if __name__ == "__main__":
    ...
    NOTE: This test requires a nanochat-style model with .generate() and .config
    attributes. It does NOT work with the GPT2 model from this repo.
    """
    import time
    # init compute
    device_type = autodetect_device_type()
    ddp, ddp_rank, ddp_local_rank, ddp_world_size, device = compute_init(device_type)
    # load the model and tokenizer — requires nanochat checkpoint_manager
    raise NotImplementedError(
        "This __main__ block requires nanochat's load_model. "
        "Use base_eval.py for evaluation instead."
    )
